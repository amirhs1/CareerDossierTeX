...
% \clubpenalty and \widowpenalty permitted, page two of this fixture opens
% on the last line of a paragraph. The runner rebuilds it that way and
% requires the check to say so; see run.sh and page-break-check.awk.
% CLUBWIDOWCONTROL: WIDOW
%
% Accepted page fill below the global floor (issue #334). Page one fills 86.9%
% of its goal and leaves 85.51pt blank, against a corpus whose every other
% governed page reaches 92.9% or better. The cause is the \clubpenalty and
% \widowpenalty pair at 10000, which leaves a paragraph of three lines or fewer
% with no legal interior break at all. #342 swept that pair over seven values
% and closed with no change: below 10000 the fill gained is paid one for one in
% stranded lines. #351 then built the successor — page-builder tolerance, which
% turns the pair into a conditional dial — measured it, and declined it: the
% dial closes this hole to 8.88pt and gains the widow line the rule exists to
% prevent, while the same tolerance costs two shipped examples 11.8 points of
% page-one fill. So this figure is accepted rather than pending, and the reason
% it is not a defect is that the eleven shipped examples lose nothing to the
% rule at all — see docs/ARCHITECTURE.md, "Page-break penalties".
%
% Delete this line if a later change closes the hole anyway. The runner fails a
% declaration that is no longer needed, so nothing has to remember to.
% PAGEFILLFLOOR: 85
...
% The two subsections below exist so the size/margin matrix renders the
% subsection pair. Every other statement fixture uses \section alone, which
% left \CDossierProseSubsectionAboveSkip and \CDossierProseSubsectionBelowSkip
% unmeasured in rendered output. The first also covers the case the token
% comment calls out: a section that opens directly with a subsection takes the
% subsection's above-gap, not the section's below-gap. No prose is added --
% the existing paragraphs are split between the two headings.
\CDossierSubsection{Adaptive Algorithms with Meaningful Error Estimates}
...
\CDossierSubsection{Durable Computational Records}
