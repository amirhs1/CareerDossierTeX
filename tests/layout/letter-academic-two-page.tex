...
%
% Issue #419 freed 3.625pt at the header-to-body boundary, which was just
% enough to pull the closing line onto page one and strand the signature name
% alone on page two -- a club/widow pair the fixture's own detector caught.
% The "Beyond research and teaching" paragraph carries one added sentence so
% the break again falls inside the body, with the same margin from the edge
% the fixture had before. That kept this fixture's break inside the body but
% left the boundary itself unprotected; #421 closed the gap by adding a
% \nobreak inside \MakeCDossierClosing, and
% letter-keeptogether-closing-two-page.tex is the fixture that forces the
% closing across the page edge on purpose to hold it.
\documentclass[family=academic]{careerdossier-letter}
\providecommand\CDossierAFourContinuationStress{}
\CDossierSetup{
  name     = {Ada Lovelace},
...
automated testing, and durable documentation practices. I have also mentored
early-career researchers preparing their own grant applications and manuscripts,
reviewed submissions for two international workshops in my field, and
contributed documentation improvements to several widely used open-source
...
\CDossierAFourContinuationStress
