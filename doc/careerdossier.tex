...
--- resume, cover letter, academic CV, and seven kinds of statement --- from one
shared profile.
...
Before \texttt{v1.0.0} the public interface may still change between minor
...
example, in \file{docs/MIGRATION.md} in the repository. Read the latter before
upgrading.

Calibrated token \emph{values} are not stable before \texttt{v1.0.0}; the token
...
\begin{itemize}
  ...
  \item \textbf{latexmk}, recommended: a resume or CV needs two passes, and
...
A complete resume, in the shape every document in the toolkit follows: load a
...
Two passes are required, and latexmk performs them. A resume and a CV record
...
Entry-metadata placement & \opt{entrymeta=column} (default) or \opt{inline}; resume and CV \\
Theme & Monochrome \\
Tagged structure & Opt-in, off by default \\
Resume class & \file{careerdossier-resume} \\
...
\midrule
\endhead
\opt{fontsize} & \opt{10pt}, \opt{11pt}, \opt{12pt} & all four & \opt{11pt} resume; \opt{12pt} others \\
\opt{margin} & \opt{normal}, \opt{narrow} & all four & \opt{narrow} resume; \opt{normal} others \\
...
\opt{entrymeta} & \opt{column}, \opt{inline} & resume, CV & \opt{column} \\
...
\paragraph{Known limitation.} The resume's default \opt{11pt} with
...
resume opens with a dense prose summary, prefer \opt{margin=normal},
...
location in the resume and CV, and the statement's application-context line.
...
Accepted by the resume and CV; the letter and statement classes do not accept it
...
discards the column the resume's layout is built around --- and because making it
...
the resume class resolves to \opt{11pt} and \opt{narrow} --- because the class
...
heading levels are available, spelled exactly as in the resume and CV:
...
\cmd{CDossierSection} in the resume and CV is entry-structured section
...
Renders the resume or CV identity block from the shared profile. It validates
...
A worked example --- a resume wanting \key{location} on a line of its own
...
Creates a resume or CV section heading followed by a full-width decorative rule.
...
A second-level heading inside a resume or CV section, for natural groupings that
...
A resume-appropriate itemized list with controlled indentation and spacing.
...
resume and the CV alike, at every supported body size, and on tagged and
...
It bolds a BibLaTeX-parsed family/given-name pair in the bibliography and does
not alter citations. Both keys are required; an incomplete declaration produces
an actionable package error.

\paragraph{Write the name as Biber parsed it.} The declaration is compared
against the name parts Biber stored, not against the text of the
\file{.bib} file, and the two differ in one respect. A name part the database
protects with braces is stored without them, so
\texttt{author = \{\{Van den Berg\}, Maria Elena\}} is declared as
\texttt{family = \{Van den Berg\}}; the doubled \texttt{family = \{\{Van den
Berg\}\}} matches nothing. Within a part, ordinary spaces are what to write:
\texttt{given = \{Maria Elena\}} matches, even though Biber separates those two
words internally with a delimiter macro rather than a space. Where a declaration
is in doubt, the \file{.bbl} file the build wrote holds the parsed spelling.

A declaration that matched no name in a bibliography the document rendered is
reported as a package warning at \cmd{end}\texttt{\{document\}}, naming the pair
it looked for. Nothing else reveals it: an unmatched declaration leaves a
document that compiles, a bibliography that renders, and one name set in the
regular weight.
...
\midrule
\endhead
\texttt{/Title} & \key{name} & \texttt{Resume – <name>}, \texttt{Cover Letter – <name>}, \texttt{Curriculum Vitae – <name>}, \texttt{Academic Cover Letter – <name>}, or the statement's title \\
\texttt{/Author} & \key{name} & \texttt{<name>} \\
\texttt{/Lang} & fixed & \texttt{en} \\
\bottomrule
\end{longtable}

The document type is part of the title so that a resume and a cover letter built
...
\begin{latexcode}
\hypersetup{ pdftitle = {Resume -- Ada Lovelace} }
\end{latexcode}
...
\begin{itemize}
  ...
  \item the resume label is \texttt{Resume}, the CV label is
...
The resume and CV classes state where a page may break rather than leaving every
...
Record (resume, CV) & \cmd{CDossierRecordHeaderBelowSkip},
...
The calibrated \emph{values} are not stable API before \texttt{v1.0.0}; the
...
\file{docs/ARCHITECTURE.md} in the repository, with the rest of the calibrated
values. It does not scale with the measure, and deliberately so: deriving it
from the line width instead was measured and rejected, because the pool a
paragraph needs tracks neither quantity.
...
Their visual definitions may evolve before \texttt{v1.0.0}.
...
fixture profiles --- industry resume, industry letter, academic CV, academic
