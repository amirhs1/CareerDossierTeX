...
- [ ] Every declared face is a real face: no silent synthetic bold or italic is
      being substituted for one the family does not supply.
- [ ] Embedded fonts are subset as expected.
- [ ] A font version that changed since the last release has had its baselines
      reviewed afresh, not carried forward.
...
- [ ] The five UA-2 fixture variants pass veraPDF, and the reports and toolchain
      ...
      (`tests/tagging/reports/`, regenerated by `make tagging`).
- [ ] At least one macOS and one Windows screen-reader reading-order check is
      ...
      ([`TESTING.md`](TESTING.md#screen-reader-reading-order-checks)).
